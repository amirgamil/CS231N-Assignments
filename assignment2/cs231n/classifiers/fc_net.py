...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        output_1, cache_1 = affine_relu_forward(X, self.params["W1"], self.params["b1"])
        scores, cache_2 = affine_forward(output_1, self.params["W2"], self.params["b2"])
        ...
        dz1, dw2, db2 = affine_backward(dy, cache_2)
        dw2 += self.reg * self.params["W2"]
        dx, dw1, db1 = affine_relu_backward(dz1, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.reg = reg
        #add 1 since output will = num_classes
        ...
        for layer in range(1, self.num_layers):
            if layer == 1:
                ...
                dim = hidden_dims[layer-2]
            self.params["W"+str(layer)] = weight_scale * np.random.randn(dim, hidden_dims[layer-1])
            self.params["b"+str(layer)] = np.zeros(hidden_dims[layer-1])
            if self.use_batchnorm:
                self.params["beta"+str(layer)] = np.random.randn(hidden_dims[layer-1])
                self.params["gamma"+str(layer)] = np.random.randn(hidden_dims[layer-1])
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers, 0, -1):
            Wlayer=self.params["W"+str(layer)]
            loss += 0.5*self.reg* np.sum(Wlayer*Wlayer)
